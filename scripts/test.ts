...
/**
 * Custom test runner for reishi.
 *
 * Default:  compact output with spinner, progress bar, and summary.
 * Verbose:  full deno test output with section headers and report.
 *
 * Usage:
 *   deno task test:run              all suites, compact
 *   deno task test:run -v           all suites, verbose
 *   deno task test:run sync         filter to sync-related suites
 *   deno task test:run sync -v      filter + verbose
 */
...
async function runCompact(suites: Suite[], results: Result[]) {
  for (let i = 0; i < suites.length; i++) {
    ...
    // After the first suite, the previous progress bar is still on screen — overwrite it
    if (i > 0) write(up(1));
