...
- **ru-R001** — Rules live as flat `.md` files directly under `rules.source` (default `~/.config/reishi/rules/`); subdirectories are not part of the rules namespace.
- **ru-R002** — Rule names are the filename stem (no `.md`); commands accept either `<name>` or `<name>.md` and strip the optional trailing `.md`.
...
- **ru-R020** — `rei rules sync` syncs all rules from source to every configured agent's `rules` path.
- **ru-R021** — Honors the sync method resolution defined in `sy-` (CLI `--method` > `[rules].sync_method` > global `sync_method`).
...
- **ru-R030** — `rei rules move <old> <new>` (alias `mv`) renames `<rules.source>/<old>.md` → `<new>.md`. Flat namespace — no project or directory dimension.
...
- **ru-R033** — Source-only operations; target cleanup is handled by `clean_on_sync` on the next sync.
