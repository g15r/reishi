...
async function main() {
	...
	try {
		...
		await test("completions fish outputs fish completion script", async () => {
			const result = await runSkillutil(["completions", "fish"]);
			if (result.code !== 0) {
		...
		await test("completions bash outputs bash completion script", async () => {
			const result = await runSkillutil(["completions", "bash"]);
			if (result.code !== 0) {
				console.log("   stderr:", result.stderr);
				return false;
			}
			return result.stdout.includes("skillutil") && result.stdout.includes("complete");
		})();

		// Test 17: completions zsh outputs zsh completion script
		await test("completions zsh outputs zsh completion script", async () => {
			const result = await runSkillutil(["completions", "zsh"]);
			if (result.code !== 0) {
				console.log("   stderr:", result.stderr);
				return false;
			}
			return result.stdout.includes("skillutil") && result.stdout.includes("compdef");
		})();

		// Test 18: completions with no shell arg shows help/error
		await test("completions with no shell arg fails", async () => {
			const result = await runSkillutil(["completions"]);
		...
		await test("completions with invalid shell name fails", async () => {
			const result = await runSkillutil(["completions", "powershell"]);
