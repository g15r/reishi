...
async function main() {
	...
	try {
		...
		await test("Init command creates skill structure", async () => {
			...
			const result = await runSkillutil([
				"init",
				"test-skill",
				"--path",
				tmpDir,
		...
		// Test 14: completion fish outputs valid fish script
		await test("completion fish outputs fish completion script", async () => {
			const result = await runSkillutil(["completion", "fish"]);
			if (result.code !== 0) {
				console.log("   stderr:", result.stderr);
				return false;
			}
			// Should contain key fish completion patterns
			const checks = [
				result.stdout.includes("complete -c skillutil"),
				result.stdout.includes("__skillutil_active_skills"),
				result.stdout.includes("__skillutil_deactivated_skills"),
				result.stdout.includes("__skillutil_no_subcommand"),
				// All subcommands present
				result.stdout.includes("-a init"),
				result.stdout.includes("-a validate"),
				result.stdout.includes("-a activate"),
				result.stdout.includes("-a deactivate"),
				result.stdout.includes("-a list"),
				result.stdout.includes("-a add"),
		...
		// Test 15: completion fish outputs nothing to stderr
		await test("completion fish writes only to stdout", async () => {
			const result = await runSkillutil(["completion", "fish"]);
			return result.code === 0 && result.stderr === "";
		})();

		// Test 16: completion bash exits 1 with friendly message
		await test("completion bash shows not-yet-available message", async () => {
			const result = await runSkillutil(["completion", "bash"]);
			return (
				result.code === 1 &&
		...
		// Test 17: completion zsh exits 1 with friendly message
		await test("completion zsh shows not-yet-available message", async () => {
			const result = await runSkillutil(["completion", "zsh"]);
			return (
				result.code === 1 &&
				result.stdout.includes("zsh") &&
				result.stdout.includes("not")
			);
		})();

		// Test 18: completion with no shell arg shows help/error
		await test("completion with no shell arg fails", async () => {
			const result = await runSkillutil(["completion"]);
			// Cliffy should error on missing required argument
			return result.code !== 0;
		})();

		// Test 19: completion with invalid shell name fails
		await test("completion with invalid shell name fails", async () => {
			const result = await runSkillutil(["completion", "powershell"]);
			return result.code !== 0;
		})();
